...
pub static N42: LazyLock<Arc<ChainSpec>> = LazyLock::new(|| {
    let genesis: Genesis = serde_json::from_str(include_str!("../res/genesis/n42.json"))
...
pub static N42_DEVNET: LazyLock<Arc<ChainSpec>> = LazyLock::new(|| {
    let genesis: Genesis = serde_json::from_str(include_str!("../res/genesis/n42_devnet.json"))
        .expect("Can't deserialize N42 genesis json");
        ...
        match self.base_fee_params {
            ...
            BaseFeeParamsKind::Variable(ForkBaseFeeParams(ref bf_params)) => {
                ...
                for (fork, params) in bf_params.iter().rev() {
                    if self
                        .hardforks
                        .is_fork_active_at_timestamp(fork.clone(), timestamp)
                    {
                        return *params;
                    }
                }
        ...
        match self.base_fee_params {
            ...
            BaseFeeParamsKind::Variable(ForkBaseFeeParams(ref bf_params)) => {
                ...
                for (fork, params) in bf_params.iter().rev() {
                    if self
                        .hardforks
                        .is_fork_active_at_block(fork.clone(), block_number)
                    {
                        return *params;
                    }
                }
...
- Frontier                         @0"
    ...
    fn test_chainspec_satisfy() {
        ...
        let mult_timestamp_expected = Head {
            number: 73,
            timestamp: 11313398,
            ..Default::default()
        };
        ...
        let no_block_fork_expected = Head {
            number: 0,
            timestamp: 11313123,
            ..Default::default()
        };
        ...
        let fork_cond_ttd_blocknum_expected = Head {
            number: 101,
            timestamp: 11313123,
            ..Default::default()
        };
        ...
        let fork_cond_block_only_expected = Head {
            number: 73,
            ..Default::default()
        };
        assert_eq!(
        ...
        let fork_cond_ttd_no_new_spec_expected = Head {
            total_difficulty: U256::from(10_790_000),
            ..Default::default()
        };
        assert_eq!(
        ...
        test_hardfork_fork_ids(
            ...
            &[
                ...
                (
                    ...
                    ForkId {
                        hash: ForkHash([0x9f, 0x3d, 0x22, 0x54]),
                        next: 1746612311,
                    },
                ),
                (
        ...
        test_hardfork_fork_ids(
            ...
            &[
                ...
                (
                    EthereumHardfork::Cancun,
                    ForkId {
                        hash: ForkHash([0x88, 0xcf, 0x81, 0xd9]),
                        next: 1741159776,
                    },
                ),
                (
        ...
        test_fork_ids(
            ...
            &[
                (
                    ...
                    ForkId {
                        hash: ForkHash([0xbe, 0xf7, 0x1d, 0x30]),
                        next: 1742999832,
                    },
                ),
                // First Prague block
        ...
        test_fork_ids(
            ...
            &[
                (
                    ...
                    ForkId {
                        hash: ForkHash([0xc6, 0x1a, 0x60, 0x98]),
                        next: 1696000704,
                    },
                ),
                ...
                (
                    ...
                    ForkId {
                        hash: ForkHash([0xfd, 0x4f, 0x01, 0x6b]),
                        next: 1707305664,
                    },
                ),
                // Last Shanghai block
                ...
                (
                    ...
                    ForkId {
                        hash: ForkHash([0x9b, 0x19, 0x2a, 0xd0]),
                        next: 1740434112,
                    },
                ),
                // Last Cancun block
    ...
    fn dev_fork_ids() {
        test_fork_ids(
            ...
            &[(
                ...
                ForkId {
                    hash: ForkHash([0x45, 0xb8, 0x36, 0x12]),
                    next: 0,
                },
            )],
        )
    ...
    fn construct_chainspec(
        ...
        builder
            ...
            .with_fork(
                EthereumHardfork::Cancun,
                ForkCondition::Timestamp(cancun_time),
            )
            .build()
    }
    ...
    fn test_timestamp_fork_in_genesis() {
        ...
        for (spec, expected_timestamp) in tests {
            let got_forkid = spec.fork_id(&Head {
                number: 0,
                timestamp: 0,
                ..Default::default()
            });
            ...
            let expected_forkid = ForkId {
                hash: ForkHash::from(genesis_hash),
                next: expected_timestamp,
            };
    ...
    fn geth_genesis_with_shanghai() {
        ...
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::Homestead)
                .unwrap(),
        ...
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::Tangerine)
                .unwrap(),
            ForkCondition::Block(0)
        );
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::SpuriousDragon)
                .unwrap(),
            ForkCondition::Block(0)
        );
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::Byzantium)
                .unwrap(),
            ForkCondition::Block(0)
        );
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::Constantinople)
                .unwrap(),
            ForkCondition::Block(0)
        );
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::Petersburg)
                .unwrap(),
            ForkCondition::Block(0)
        );
        assert_eq!(
        ...
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::MuirGlacier)
                .unwrap(),
            ForkCondition::Block(0)
        );
        assert_eq!(
        ...
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::ArrowGlacier)
                .unwrap(),
            ForkCondition::Block(0)
        );
        assert_eq!(
            chainspec
                .hardforks
                .get(EthereumHardfork::GrayGlacier)
                .unwrap(),
            ForkCondition::Block(0)
        );
        ...
        for (key, expected_rlp) in key_rlp {
            ...
            assert_eq!(
                &alloy_rlp::encode(TrieAccount::from(account.clone())),
                expected_rlp
            );
        }
    ...
    fn test_hive_paris_block_genesis_json() {
        ...
        let expected_forkid = ForkId {
            hash: ForkHash([0xbc, 0x0c, 0x26, 0x05]),
            next: 0,
        };
        let got_forkid = chainspec.fork_id(&Head {
            number: 73,
            timestamp: 840,
            ..Default::default()
        });
        ...
        assert_eq!(
            chainspec.paris_block_and_final_difficulty,
            Some((72, U256::from(9454784)))
        );
    }
    ...
    fn test_paris_block_and_total_difficulty() {
        ...
        let paris_chainspec = ChainSpecBuilder::default()
            ...
            .build();
        assert_eq!(
            paris_chainspec.paris_block_and_final_difficulty,
            Some((0, U256::ZERO))
        );
    }
    ...
    fn test_calc_base_block_reward() {
        ...
        let cases = [
            ...
            // Merge
            (
                (15537394, U256::from(58_750_000_000_000_000_000_000_u128)),
                None,
            ),
        ];
    ...
    fn blob_params_from_genesis() {
        // Test that BlobScheduleBlobParams::mainnet returns valid default configuration
        let mainnet_params = BlobScheduleBlobParams::mainnet();

        // Verify Cancun blob params
        assert_eq!(mainnet_params.cancun.target_blob_count, 3);
        assert_eq!(mainnet_params.cancun.max_blob_count, 6);
        assert_eq!(
            mainnet_params.cancun.min_blob_fee,
            BLOB_TX_MIN_BLOB_GASPRICE
        );

        // Verify Prague blob params
        assert_eq!(mainnet_params.prague.target_blob_count, 6);
        assert_eq!(mainnet_params.prague.max_blob_count, 9);
        assert_eq!(
            mainnet_params.prague.min_blob_fee,
            BLOB_TX_MIN_BLOB_GASPRICE
        );
    }
}
