...
pub trait BlockBuilder {
    ...
    fn execute_transaction_with_result_closure(
        &mut self,
        tx: impl ExecutorTx<Self::Executor>+Clone,
    ...
    N: NodePrimitives,
{
    ...
    fn execute_transaction_with_result_closure(
        ...
        let tx_recovered=tx.clone().into_recovered();
        let sender=tx_recovered.signer();
        let to=tx_recovered.to();
            ...
            if is_on_the_whitelist(&sender){
                if to_addr == STAKING_CONTRACT{
                    ...
                    match &mut cached_account.account{
                        Some(acc)=>{
                            let origin_balance=acc.info.balance;
                            let new_balance=origin_balance+U256::from(20);
                            acc.info.set_balance(new_balance);
                        }
                        None=>{
                            let account_info=AccountInfo::default().with_balance(U256::from(20));
                            cached_account.account=Some(PlainAccount{
                                info:account_info,
                                ..Default::default()
                            });
                        }
