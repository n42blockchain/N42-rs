...
use alloy_primitives::{Address, B256, B64, Bloom, Bytes, Sealable, Sealed, U256, keccak256};
...
impl<'de> serde::Deserialize<'de> for ExecutionPayloadInputV2 {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        struct Helper {
            ...
            // N42: carried so the APoS seal survives deserialization. Both are
            // `default` so payloads from peers that omit them still decode.
            #[serde(default)]
            difficulty: U256,
            #[serde(default)]
            nonce: B64,
...
pub struct ExecutionPayloadV1 {
    ...
    /// N42: block difficulty.
    ///
    /// Not part of upstream's engine-API payload. APoS carries a Clique-style
    /// seal, so difficulty and nonce have to survive the payload round trip.
    /// Both default and skip-if-zero, so encodings stay wire-compatible with
    /// peers that do not send them.
    ...
    pub difficulty: U256,
    /// N42: block nonce. See [`Self::difficulty`].
...
impl ExecutionPayloadV1 {
    ...
    pub fn into_block_raw_with_transactions_root_opt(
        ...
        // N42: no MAXIMUM_EXTRA_DATA_SIZE check. APoS stores the vanity prefix, the
        // signer list at epoch checkpoints, and the 65-byte seal in extra_data, which
        // is far past Ethereum's 32-byte cap. Enforcing it rejects every N42 block.
        ...
        let header = Header {
            ...
            // N42: carried from the payload rather than defaulted — the APoS seal
            // lives in these two fields, so defaulting them changes the block hash.
