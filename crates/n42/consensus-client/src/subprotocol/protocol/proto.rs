...
use alloy_primitives::{bytes::{Buf, BufMut, BytesMut}, BlockHash};
...
pub(crate) enum CustomRlpxProtoMessageId {
    ...
    PingMessage = 0x02,
    PongMessage = 0x03,
    BeaconBlockRequest = 0x04,
    BeaconBlockResponse = 0x05,
}
...
pub(crate) enum CustomRlpxProtoMessageKind {
    ...
    PingMessage(String),
    PongMessage(String),
    BeaconBlockRequest(BlockHash),
    BeaconBlockResponse(BeaconBlock),
}
...
impl CustomRlpxProtoMessage {
    ...
    /// Creates a beacon block request
    pub fn beacon_block_request(block_hash: BlockHash) -> Self {
        Self {
            message_type: CustomRlpxProtoMessageId::BeaconBlockRequest,
            message: CustomRlpxProtoMessageKind::BeaconBlockRequest(block_hash),
        }
    }

    /// Creates a beacon block response
    pub fn beacon_block_response(beacon_block: BeaconBlock) -> Self {
        Self {
            message_type: CustomRlpxProtoMessageId::BeaconBlockResponse,
            message: CustomRlpxProtoMessageKind::BeaconBlockResponse(beacon_block),
        }
    }
    ...
    pub fn encoded(&self) -> BytesMut {
        debug!(target: "reth::cli", ?self, "encoded");
        ...
        match &self.message {
            ...
            CustomRlpxProtoMessageKind::BeaconBlockRequest(block_hash) => {
                buf.put(block_hash.as_ref());
            }
            CustomRlpxProtoMessageKind::BeaconBlockResponse(beacon_block) => {
                buf.put(&*bincode::serialize(&beacon_block).unwrap());
            }
        }
        debug!(target: "reth::cli", ?buf, "encoded");
        buf
    }
    ...
    pub fn decode_message(buf: &mut &[u8]) -> Option<Self> {
        ...
        let message_type = match id {
            ...
            0x04 => CustomRlpxProtoMessageId::BeaconBlockRequest,
            0x05 => CustomRlpxProtoMessageId::BeaconBlockResponse,
            _ => return None,
        };
        let message = match message_type {
            ...
            CustomRlpxProtoMessageId::BeaconBlockRequest => CustomRlpxProtoMessageKind::BeaconBlockRequest(
                BlockHash::try_from(&buf[..]).unwrap(),
            ),
            CustomRlpxProtoMessageId::BeaconBlockResponse => CustomRlpxProtoMessageKind::BeaconBlockResponse(
