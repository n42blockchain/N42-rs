...
mod tests {
    ...
    fn test_apos_config_default() {
        ...
        assert_eq!(config.reward_limit, U256::from(0x6F05B59D3B20000_u64));
        assert_eq!(config.deposit_contract, Address::ZERO);
    }

    #[test]
    fn test_apos_config_custom() {
        let deposit = address!("1234567890123456789012345678901234567890");
        ...
        assert_eq!(config.epoch, 5000);
        assert_eq!(config.deposit_contract, deposit);
    }
    ...
    #[test]
    fn test_apos_config_rlp() {
        let config = APosConfig::default();
        let mut buf = Vec::new();
        alloy_rlp::Encodable::encode(&config, &mut buf);
        let decoded = alloy_rlp::Decodable::decode(&mut buf.as_slice()).unwrap();
    ...
    fn test_vote_rlp() {
        ...
        let decoded: Vote = alloy_rlp::Decodable::decode(&mut buf.as_slice()).unwrap();
    ...
    fn create_test_signers() -> Vec<Address> {
        vec![
    ...
    fn create_test_snapshot() -> Snapshot {
        ...
        Snapshot::new_snapshot(config, 100, hash, create_test_signers())
    }
    ...
    fn test_snapshot_signers() {
        let snap = create_test_snapshot();
        let signers = snap.signers();
        assert_eq!(signers.len(), 3);
        assert_eq!(signers[0], address!("1111111111111111111111111111111111111111"));
    }

    #[test]
    fn test_snapshot_valid_vote_authorize_new_signer() {
        let snap = create_test_snapshot();
        let new_signer = address!("4444444444444444444444444444444444444444");
        // Voting to authorize a new signer should be valid
        assert!(snap.valid_vote(new_signer, true));
        // Voting to deauthorize a non-existent signer should be invalid
    ...
    fn test_snapshot_valid_vote_deauthorize_existing_signer() {
        let snap = create_test_snapshot();
        let existing_signer = address!("1111111111111111111111111111111111111111");
        // Voting to deauthorize an existing signer should be valid
        assert!(snap.valid_vote(existing_signer, false));
        // Voting to authorize an existing signer should be invalid
        assert!(!snap.valid_vote(existing_signer, true));
    }

    #[test]
    fn test_snapshot_cast_valid_vote() {
        let mut snap = create_test_snapshot();
        let new_signer = address!("4444444444444444444444444444444444444444");

        // Cast a valid vote to add a new signer
    ...
    fn test_snapshot_cast_invalid_vote() {
        ...
        // Trying to authorize an already authorized signer should fail
        assert!(!snap.cast(existing_signer, true));
        assert!(!snap.tally.contains_key(&existing_signer));
    }
    ...
    fn test_snapshot_uncast_vote() {
        ...
        // Uncast last vote - should remove from tally
        assert!(snap.uncast(new_signer, true));
        assert!(!snap.tally.contains_key(&new_signer));
    }
    ...
    fn test_snapshot_uncast_wrong_type() {
        ...
        // Try to uncast a deauthorize vote (wrong type)
    ...
    fn test_snapshot_uncast_nonexistent() {
        let mut snap = create_test_snapshot();
        let unknown = address!("5555555555555555555555555555555555555555");

        // Try to uncast a vote that doesn't exist
        assert!(!snap.uncast(unknown, true));
    }
    ...
    fn test_snapshot_inturn() {
        ...
        // With 3 signers, inturn cycles through them
        // Block 1: signer at index (1-1) % 3 = 0
        assert!(snap.inturn(1, &signer1));
        assert!(!snap.inturn(1, &signer2));
        assert!(!snap.inturn(1, &signer3));

        // Block 2: signer at index (2-1) % 3 = 1
        assert!(!snap.inturn(2, &signer1));
        assert!(snap.inturn(2, &signer2));
        assert!(!snap.inturn(2, &signer3));

        // Block 3: signer at index (3-1) % 3 = 2
        assert!(!snap.inturn(3, &signer1));
        assert!(!snap.inturn(3, &signer2));
        assert!(snap.inturn(3, &signer3));

        // Block 4: back to signer at index (4-1) % 3 = 0
        assert!(snap.inturn(4, &signer1));
    }
