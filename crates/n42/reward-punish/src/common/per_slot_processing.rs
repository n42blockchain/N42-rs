...
use crate::Hash256;
use crate::spec::Spec as ChainSpec;
...
use crate::common::per_epoch_processing::process_epoch as per_epoch_processing;
...
pub fn per_slot_processing<E: EthSpec>(
    ...
    let summary = if state.slot() > spec.genesis_slot
        && state.slot().safe_add(1)?.safe_rem(E::slots_per_epoch())?.as_u64() == 0
    {
    ...
    if state.slot().safe_rem(E::slots_per_epoch())?.as_u64()== 0 {
