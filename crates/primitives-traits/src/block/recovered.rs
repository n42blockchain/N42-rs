...
use alloy_eips::{eip1898::BlockWithParent, eip2718::Encodable2718, BlockNumHash};
...
impl<B: Block> RecoveredBlock<B> {
    ...
    pub fn find_indexed(&self, tx_hash: B256) -> Option<IndexedTx<'_, B>>
    where
        <B::Body as BlockBody>::Transaction: Encodable2718,
    {
        self.body()
            .transactions_iter()
            .enumerate()
    ...
    <B::Body as BlockBody>::Transaction: Encodable2718,
{
    ...
    pub fn tx_hash(&self) -> B256 {
        Encodable2718::trie_hash(self.tx)
    }
    ...
    pub fn meta(&self) -> alloy_consensus::transaction::TransactionMeta {
        alloy_consensus::transaction::TransactionMeta {
            tx_hash: Encodable2718::trie_hash(self.tx),
...
/// RPC-compatible block conversions.
