    ...
    F::Provider: DBProvider + TrieReader + StageCheckpointReader + PruneCheckpointReader + BlockNumReader,
{
    ...
    fn database_provider_ro(&self) -> ProviderResult<OverlayStateProvider<F::Provider>> {
        ...
        // Check if reverts are needed
        let should_collect_reverts = if let Some(from_block) = self.get_block_number(&provider)? {
        ...
        let (trie_updates, hashed_state) = if should_collect_reverts {
            let from_block = self.get_block_number(&provider)?.unwrap();
