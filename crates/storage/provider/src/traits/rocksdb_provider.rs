...
pub trait RocksDBProviderFactory {
    ...
    ///
    /// The closure should return a tuple of `(result, Option<batch>)`. If the batch is
    /// `Some`, it will be set as the pending batch to be committed later.
    ///
        ...
        F: FnOnce(RocksBatchArg<'_>) -> ProviderResult<(R, Option<RawRocksDBBatch>)>,
    {
        #[cfg(all(unix, feature = "rocksdb"))]
        {
            ...
            let (result, maybe_batch) = f(batch)?;
