...
pub use crate::{
    ...
    traits::*,
    validate::{
        EthTransactionValidator, N42TransactionValidator, TransactionValidationOutcome, TransactionValidationTaskExecutor,
...
/// Type alias for N42 transaction pool
pub type N42TransactionPool<Client, S> = Pool<
    TransactionValidationTaskExecutor<N42TransactionValidator<Client, EthPooledTransaction>>,
    CoinbaseTipOrdering<EthPooledTransaction>,
    S,
>;
...
impl<Client, S> N42TransactionPool<Client, S>
where
    Client:
        ChainSpecProvider<ChainSpec: EthereumHardforks> + StateProviderFactory + Clone + 'static,
    S: BlobStore,
{
    /// Returns a new [`Pool`] that uses the default [`TransactionValidationTaskExecutor`] when
    /// validating [`EthPooledTransaction`]s with N42 validation logic and ords via [`CoinbaseTipOrdering`]
    ///
    /// # Example
    ///
    /// ```
    ...
    /// # fn t<C>(client: C)  where C: ChainSpecProvider<ChainSpec: EthereumHardforks> + StateProviderFactory + Clone + 'static {
    /// let blob_store = InMemoryBlobStore::default();
    /// let pool = Pool::n42_pool(
    ///     TransactionValidationTaskExecutor::n42(
    ///         client,
    ///         blob_store.clone(),
    ///         TokioTaskExecutor::default(),
    ///     ),
    ...
    pub fn n42_pool(
        validator: TransactionValidationTaskExecutor<
            N42TransactionValidator<Client, EthPooledTransaction>,
        >,
        blob_store: S,
        config: PoolConfig,
    ) -> Self {
