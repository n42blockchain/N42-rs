...
use crate::{
    blobstore::BlobStore,
    validate::{EthTransactionValidatorBuilder, N42TransactionValidatorBuilder, TransactionValidatorError},
    EthTransactionValidator, N42TransactionValidator, PoolTransaction, TransactionOrigin, TransactionValidationOutcome,
...
impl TransactionValidationTaskExecutor<()> {
    ...
    /// Convenience method to create a [`N42TransactionValidatorBuilder`]
    pub fn n42_builder<Client>(client: Client) -> N42TransactionValidatorBuilder<Client> {
        N42TransactionValidatorBuilder::new(client)
    }
}
...
impl<Client, Tx> TransactionValidationTaskExecutor<N42TransactionValidator<Client, Tx>> {
    /// Creates a new instance for the given client
    ///
    /// This will spawn a single validation tasks that performs the actual validation.
    /// See [`TransactionValidationTaskExecutor::n42_with_additional_tasks`]
    pub fn n42<T, S: BlobStore>(client: Client, blob_store: S, tasks: T) -> Self
    where
        T: TaskSpawner,
    {
        Self::n42_with_additional_tasks(client, blob_store, tasks, 0)
    }
    ...
    ///   - eip1559
    ///   - eip2930
    ///   - whitelist checking for staking contract
    ///
    /// This will always spawn a validation task that performs the actual validation. It will spawn
    /// `num_additional_tasks` additional tasks.
    pub fn n42_with_additional_tasks<T, S: BlobStore>(
        client: Client,
        blob_store: S,
        tasks: T,
        num_additional_tasks: usize,
