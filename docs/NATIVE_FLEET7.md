...
8500fed04).** The follower's root phase at 147,000 accounts was 190 ms:
...
real trigger.**
    ...
    twig index: key-prefix hasher + reserve; follower graft skips    N1,N3     228,189 (0.714 s) x2      16.6M x2          +2.4% over O2 222,755;
      the cache insert (bb7fb4a6e)                                   O2,O3     222,755 / 186,092         15.5M / 14.6M       import 474 vs 488-503 ms
    provider hashes a large post-state over rayon chunks             N4        246,132 (0.652 s)         (window 2-3 stalled) leader finish 249 -> 116 ms,
      (a8a437cc2 / 04115182e), also on the builder's `finish`                                                                build 609 -> 365
...
`N42_FOLLOWER_SENDER_GROUPS=1` (c02d01134) against connected components,
...
handover stalls. `--straggler-grace-ms` (F7_STRAGGLER_GRACE_MS; 5e59ae4e8,
f6a45559a, 6fca3dff7) makes the leader wait, after a view is decided,
...
until the chain settles its height (cfe7a64c2; loop93 validates it).
...
and hugeprep before every leg. S = the loop95 binary (HEAD a87d00459), B =
...
that is cores the ingest wanted. Second attempt (b14304a73): remember the
...
`carry_ms` from b14304a73, and nothing reads it before the next block.
...
44 that was inferred, and `N42_CARRY_ASYNC=1` (fef4c27ef) moves it behind the
...
loop110 (48372fef8 as built at 18:05): S1 202,634 / B1 217,246 / S2 232,707 / B2
...
stale-request guard (18a8f4f40) on the loop115 binaries otherwise, W S1 B1 S2 B2:
...
made it pay was not the on-seal path itself (48372fef8 read 233-244k against 266-271k)
...
page tables populated (`230363f8f`), E against the heap A, retention 16 on both:
...
at 21.11M.** Step 3a (`ef280b1c1`): the delta says which slots below its base flipped and to what
...
records with its own seek and write syscall; fixed after this round (`aff1d91f0`: appends go to
...
3b/4 (`8b7539bea`: `forest.ckpt` is the cursor and the active bits, the delta names its appended
range by its bounds, fsync before the delta, restart from the file) with the appends buffered
and written in one call (`aff1d91f0`). E legs the entry file, A legs the heap, retention 16,
tenure 64:
...
and 298k on window 1.** Step 5 (`a2616faea`: a twig whose slots are all dead and outside the
retention window keeps only its root and bits) and the fsync of the entry file's appends moved
outside the forest lock (`c7e40e23b`). T = entry file with trimming, N = entry file without
...
the reason it could not work.** `N42_VOTE_BEFORE_IMPORT=1` (`a9d0adc92`): under the H2-v4
...
through `execute`, so the leader never finalised (`c1093df95` narrows it to an import in
...
bench now also drops gov5's datadir pages before a leg (`98144d230`) -- which, measured
...
(`dcc6e4f85`), V1 collected its votes in 5-21 ms instead of 499 and made 59 blocks in window 1
...
and the next block's check waited the whole parent timeout. Fixed (2a80b6da2): a commit for a
...
that fell behind at a tenure change stalled the fleet 31 s. Fixed (26d9a2dbe): the imports in
...
window, the whole difference between 46 and 56 blocks. Fixed (eabc68599): the proposal is
...
two legs a build:
    ...
    loop143 A1   b0bb8b279 (audit)      284,806 (54)  119,557  225,656  tenure timeouts: a queued block was not "importing"
    loop143 A2   b0bb8b279              300,210 (56)  142,924   28,041  window 3 at 17% occupancy (see loop144 A2)
    loop144 A1   5c38c8798              300,245 (56)        0        0  node0's head stuck at 173 while consensus ran on
    loop144 A2   5c38c8798              316,429 (59)    5,468        0  the leader's blocks 163k, 126k, 32k, 0; flood stalled
    loop145 A1   98ceadeb4              296,666 (55)  211,481  193,739  full round, 21.07M
    loop145 A2   98ceadeb4              242,516 (46)  224,726  203,987  full round, 20.14M (0.65 s window-1 cycle)
...
(+36 ms) -- `5c38c8798`. loop144 A1 is the liveness defect the fix uncovered: the commit for a
...
one's, and a refused forkchoice is a log line (`98ceadeb4`).
...
every round since ccf0a4f31 (2026-09-02) and is part of why windows 2-3 read well under window 1.
Fixed in `9465fdb9f`: the build's start returns the run's sender to the front of the arrival
...
327,484 / 238,126 and 22.33M.** The same binary plus `9465fdb9f`, two legs:
...
rejected. Both are fixed in `4e55c33ed` (the sealed block is found, not taken, and a payload
...
not recorded now (`27d13a625`); loop152 measures it.
...
The hand-off now moves the engine's head to the sibling's parent first (`df0e9f4c2`), so the
...
last committed (`6f5007b58`). loop154 measures both.
...
Four legs in a row since `df0e9f4c2` ran to the end with no header rejected (loop154 A1 through
...
`rep-c`, the configuration as committed at `078925970` (direct push, body
...
### The tenure's three-round record (async ingest, pool 489k, tree at a955bc0f7)
...
Branch `feat/tenure-leader` (bd2032ded), tenure 16, 250 ms pacing, `--ingest-all`,
...
4a661bbaf — such a group's members go up as leaves — with a test over every
...
Three more rounds of the other branch's gate watcher (cff12cb9e: one
...
leg that no collapsing round had: 6172677a0 -- the body channel's pooled
...
transaction into the executor. Both removed (7182c300c; the read stays
...
once less than a transfer's gas is left (0b02b6cb2, 56428a752 for the
...
(8e1a122f3). Own import 127 -> 71 ms.
...
`N42_FAST_TRANSFER=1` (1b2f48cae) applies a plain transfer -- a call with
...
cache (`N42_BUILDER_PREFETCH=<n>`, 56428a752; a state provider per chunk,
...
refusal counters (9eb61cafe) and a `perf` profile of the leader's build
...
now takes the node's factory (409fc6591). Same binary, 16 slots, the
flag alone:
...
03f6967e3), so it is not the path. Cause not established -- a
...
(91c99d962: alloy's address hasher in the queue, one clock read per
...
(ce793c66d), and bookended; registered before the legs: build level
...
so the split is now timed with rdtsc, cef2e7aa8.)
...
Three legs back to back on 6172677a0 + the supply commits, direct
...
collapsing rounds (gate1-3, direct3-5) did not have, is 6172677a0 -- the
...
A-B-A on one binary (feat/supply-direct f0f6f7bdb: 6172677a0 + the supply
...
the same fast path. `N42_FOLLOWER_EXEC_PROBE=1` (7f2a0ff2b) executes
...
proved (acc3838a9), and a read cache carried from the previous block's
post-state (7e16bf1c8). Per block: convert 35, header 1, senders 37,
...
previous parent, trusted and proposed (b759c48f3 checks the delivered
parent), and the QC-moved race (d5613db61) -- see `N42_26_PORT.md`.
...
(`N42_TX_QUEUE_BATCH`, b7765bedc), to be measured.
...
timeout). The driver's parent check (b759c48f3) caught the first form and
rebuilt; fc32c5c3c removes the cause: the connection is taken out of the
...
before any response (315197d24; a server-closed keep-alive connection cost
a view each time), the queue takes transactions in batches under one lock
(`N42_TX_QUEUE_BATCH`, b7765bedc), returns what a superseded build took
minus what the chain mined meanwhile (da182a8b2, 7bc369ed4 -- the first
...
(`N42_TX_QUEUE_DRAINER=1`, d5f24c2ae, to be measured).
...
`recv`, which read like a generator at its signing ceiling. 5a9b51eb9 signs
...
`slot_wait_us_per_tx` outside it (0058e7d29; the first cut instrumented only
...
5801f14b4 and 1e6d68ba6 time both ends. The node's stats line adds, per
...
as 32 x 100), one binary 1e6d68ba6:
...
3,000 a worker; the flood's frame cap is 2,000 since 56f60f853). The
