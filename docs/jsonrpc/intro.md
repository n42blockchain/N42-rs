...
| [`reth`](./reth.md)                                   | The `reth` API provides reth-specific methods.                                                         | No        |
...
```bash
n42 node --http --http.corsdomain https://mycoolapp.rs
```

Alternatively, if you want to allow any domain, you can pass `*`:

```bash
n42 node --http --http.corsdomain "*"
```
...
```bash
RUST_LOG=info n42 node --http --http.api "admin,debug,eth,net,trace,txpool,web3,rpc,reth,ots"
```
