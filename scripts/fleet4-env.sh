#!/usr/bin/env bash
# The four-node fleet: the whole of the switch.
#
#   source scripts/fleet4-env.sh          # bench tier (the default)
#   source scripts/fleet4-env.sh lean     # the production-shaped chain
#
# then `scripts/fleet7-bench.sh --tag …`, `scripts/fleet7.sh up --fresh`,
# `scripts/fleet7.sh print` -- unchanged, every one of them.
#
# WHY four. Seven nodes at 16 physical cores each read 429-443k TPS and the
...
# (`docs/FLEET7_PLAN_V4.md` sections 2o, 2p). Four nodes at 28 physical cores
# each change that budget without changing a line of the node: the same
# binaries, a four-validator genesis, quorum 3 of 4.
#
...
# cover -- so a fleet7 datadir would be accepted by a fleet4 node and diverge
...
if [[ $_f4_tier == bench ]]; then
  : "${F7_ROOT:=/data/blockchain/rust-fleet4-bench}"
  : "${F7_GENESIS:=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/crates/chainspec/res/genesis/n42_fleet4_bench.json}"
else
  : "${F7_ROOT:=/data/blockchain/rust-fleet4}"
  : "${F7_GENESIS:=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/crates/chainspec/res/genesis/n42_fleet4.json}"
fi

# Four members, quorum 3 (f = 1). `fleet7-env.sh`'s `f7_quorum` computes it;
...
# node that outlived its round stops a fleet4 round instead of quietly running
...
# key in both fleets and `crates/chainspec/res/genesis/n42_fleet4*.json` is the
# seven-node file's validator list truncated to four
