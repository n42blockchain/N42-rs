...
export F7_BLOCK_INTERVAL_MS=${F7_BLOCK_INTERVAL_MS:-350}
...
for ((fp = 0; fp < FLOOD_PROCS; fp++)); do
  ...
  setsid $FLOOD_PIN "$F7_BIN/examples/tx_flood" --rpc "$RPCS" --chain-id "$CHAIN" \
    ...
    --conc "$fp_conc" --rpcbatch "$RPCBATCH" ${F7_FLOOD_RATE:+--rate "$(( F7_FLOOD_RATE / FLOOD_PROCS ))"} ${F7_FLOOD_REPLAY:+--replay "$F7_FLOOD_REPLAY"} ${F7_FLOOD_CLAIM:+--claim-sender} ${F7_FLOOD_GATEWAY_KEY:+--gateway-key "$F7_FLOOD_GATEWAY_KEY"} ${F7_FLOOD_WINDOW:+--window "$F7_FLOOD_WINDOW"} $SHARD \
