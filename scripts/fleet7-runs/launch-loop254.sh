#!/bin/bash
# Waits for the box, builds the node for this CPU into target/native, then runs loop254.
...
if ! RUSTFLAGS="-C target-cpu=native" nice -n 19 cargo build -j16 --release --target-dir target/native -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > target/fleet-runs/build-native-loop254.log 2>&1; then echo "NATIVE BUILD FAILED"; tail -5 target/fleet-runs/build-native-loop254.log; echo ALLDONE; exit 1; fi
echo "native built at $(date +%H:%M)"
bash target/fleet-runs/run-loop254.sh
