#!/usr/bin/env bash
# loop170: a block that will seal early builds its receipts beside the parallel step instead of one executor commit
# per transfer (N42_DIRECT_RECEIPTS=1; 53 ms a full block on the leader's serial chain). Legs in the new baseline
# (reads on, hashed tables off, merged Ed25519 batch): D0x without it, D1x with it, and J1x with
# MALLOC_CONF=thp:always,oversize_threshold:0 -- jemalloc 5.3 purges every free of >= 8 MiB at once (its huge arena),
# and loop167's leader spent 24% of its builder thread and 25% of its engine thread in that madvise. Per leg: stall
# counts, seal-first logs that took the direct receipts, the guard's errors, invalid blocks. Tests and the build run
# BEFORE the claim (niced, while the box is busy); every leg under a hard timeout; no new leg after 75 minutes of the
# claim; the claim is released on any exit.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop170.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop170$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop170$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
J='MALLOC_CONF=thp:always,oversize_threshold:0'
leg D0a F7_BIN=$NEW $C $R N42_QMDB_READS=on N42_HASHED_TABLES=off
leg J1a F7_BIN=$NEW $C $R N42_QMDB_READS=on N42_HASHED_TABLES=off $J
leg D1a F7_BIN=$NEW $C $R N42_QMDB_READS=on N42_HASHED_TABLES=off N42_DIRECT_RECEIPTS=1
leg D0b F7_BIN=$NEW $C $R N42_QMDB_READS=on N42_HASHED_TABLES=off
leg J1b F7_BIN=$NEW $C $R N42_QMDB_READS=on N42_HASHED_TABLES=off $J
leg D1b F7_BIN=$NEW $C $R N42_QMDB_READS=on N42_HASHED_TABLES=off N42_DIRECT_RECEIPTS=1
cleanup
