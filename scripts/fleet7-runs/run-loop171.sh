#!/usr/bin/env bash
# loop171: jemalloc's huge arena (allocations >= 8 MiB) purges every free at once, and loop170's J legs showed what
# that costs: with MALLOC_CONF=thp:always,oversize_threshold:0 the round went 21.3 -> 25.0/25.6M transactions and the
# leader's build 381 -> 312/300 ms, but the box's free memory fell to 7.4/10.3 GB against 40 GB. A2 bounds that with
# dirty_decay_ms:2000 and a background thread to purge off the application's threads. Every leg carries
# N42_DIRECT_RECEIPTS=1 (loop170: receipts 61 -> 22-30 ms, 336 blocks, 0 guard errors, 0 invalid blocks), so the legs
# differ only in the allocator. A0 = today's setting, A1 = loop170's J, A2 = A1 bounded. Tests and the build run
# BEFORE the claim; every leg under a hard timeout; no new leg after 75 minutes of the claim; released on any exit.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop171.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop171$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop171$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
A1='MALLOC_CONF=thp:always,oversize_threshold:0'
A2='MALLOC_CONF=thp:always,oversize_threshold:0,dirty_decay_ms:2000,background_thread:true'
D='N42_QMDB_READS=on N42_HASHED_TABLES=off N42_DIRECT_RECEIPTS=1'
leg A0a F7_BIN=$NEW $C $R $D
leg A1a F7_BIN=$NEW $C $R $D $A1
leg A2a F7_BIN=$NEW $C $R $D $A2
leg A0b F7_BIN=$NEW $C $R $D
leg A1b F7_BIN=$NEW $C $R $D $A1
leg A2b F7_BIN=$NEW $C $R $D $A2
cleanup
