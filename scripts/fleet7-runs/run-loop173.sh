#!/usr/bin/env bash
# loop173: the leader's profile on the new footing (loop172 recorded nothing -- its pid match wanted "node0 ",
# and a node's argument is "--datadir .../node0/el"; the throughput of its two legs stands: build 310 and 332 ms,
# 45.4 and 26.5 GB of the box free). loop167's profile was taken before the allocator change and 24%
...
# release build does not. One node is recorded for 100 s from 15 s after the funding, which spans the flood: a node
# leads roughly one tenure in seven, so this catches a tenure's worth of build samples. Tests and the build are done;
# every leg under a hard timeout; released on any exit.
...
run() { local tag=$1; shift
  ...
  ( n=0; until grep -q 'funding' $B/bench-$tag/flood.log 2>/dev/null; do sleep 1; n=$((n+1)); [ $n -gt 300 ] && exit 0; done
    sleep 15; p=$(ps -eo pid,args | grep -E '/n4[2] node' | grep -F "rust-fleet7-bench/node$PERF_NODE/el" | awk '{print $1}' | head -1)
    echo "perf: node$PERF_NODE pid=${p:-none} at $(date +%H:%M:%S)"
    [ -n "$p" ] && nice -n 5 perf record -F 499 -g -p $p -o $B/bench-$tag/perf-node$PERF_NODE.data -- sleep 100
    echo "perf done $? at $(date +%H:%M:%S): $(ls -la $B/bench-$tag/perf-node$PERF_NODE.data 2>&1 | tail -1)" ) > $S/perf-$tag.log 2>&1 &
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop173$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop173$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
