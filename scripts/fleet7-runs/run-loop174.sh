#!/usr/bin/env bash
# loop174: the follower's import is the longest single step on the fleet now (336-452 ms a full block against the
# leader's 310-330 ms build), and it gates both the vote and a tenure change -- loop173's tenure changes cost 2.3 s
# each, the new leader waiting out its own import of the last leader's block. Its largest phase is the execution
# (139-200 ms) where the leader executes the same 163,000 transfers in 81: the follower partitions by connected
# component, which on random transfers is ~200 groups with one giant one that sets the wall time, while the leader
# groups by sender. `N42_FOLLOWER_SENDER_GROUPS=1` gives the follower the leader's grouping; a transfer that only
# another sender's credit in the same block would fund sends the block to the serial path, so the legs watch
# exec_ms and total_ms medians, not just the round. F0 = component groups, F1 = sender groups, three pairs, on the
# adopted allocator and the direct receipts. Tests and the build run BEFORE the claim; released on any exit.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop174.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop174$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop174$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
G='N42_FOLLOWER_SENDER_GROUPS=1'
leg F0a F7_BIN=$NEW $C $R $D $A2
leg F1a F7_BIN=$NEW $C $R $D $A2 $G
leg F0b F7_BIN=$NEW $C $R $D $A2
leg F1b F7_BIN=$NEW $C $R $D $A2 $G
leg F0c F7_BIN=$NEW $C $R $D $A2
leg F1c F7_BIN=$NEW $C $R $D $A2 $G
cleanup
