#!/usr/bin/env bash
# loop175: the graft, folded beside the execution. Each batch's bundle used to be kept and grafted into the block's
# state after the execution -- 60 ms of the leader's serial chain and the same again inside a follower's import
# (loop173-174: the build thread is 37% byte movement, and the graft is what moves it). With N42_GRAFT_STREAM=1 a
# batch folds its bundle into a staged bundle of the graft's own as it finishes, on its own thread, and the block's
# state takes the result in one move at the end; a batch that fails leaves the block's state untouched, as before.
# The equivalence is in the unit tests (`a_staged_graft_lands_where_the_graft_does`, and the serial-executor
# comparisons run with the flag on); these legs ask what it costs on the fleet. S0 = today, S1 = streamed, three
# S0 = today, S1 = streamed, S2 = streamed with the follower grouping by sender too (a follower's component
# partition leaves one giant group whose bundle lands last, which the streaming cannot hide; by sender the batches
# are of a size, as the leader's are). Two legs each, on the adopted allocator and the direct receipts. Tests and
# the build run BEFORE the claim.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop175.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop175$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop175$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
