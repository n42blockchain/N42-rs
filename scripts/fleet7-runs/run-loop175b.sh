#!/usr/bin/env bash
# loop175b: the same six legs as loop175 (S0 today, S1 the streamed graft, S2 streamed with the follower grouping by
# sender, two each), on the binary loop175 already tested and built at 00:25 from 2e31a7845 -- it refuses to run if
# any source is newer than that binary. The waiting rule is the change: loop175 sat out a 16-hour window because its
# quiet check wanted a load average under 8, and this box has 256 CPUs, so its own build kept it out. What decides
# now is whether anyone else claims the box or has a heavy process on it; the load average is only a backstop.
...
trap cleanup EXIT
if [ ! -x $NEW/n42 ]; then echo "NO BINARY at $NEW"; echo ALLDONE; exit 1; fi
newer=$(find crates bin -name '*.rs' -newer $NEW/n42 2>/dev/null | head -3)
if [ -n "$newer" ]; then echo "SOURCE NEWER THAN THE BINARY ($newer); build first"; echo ALLDONE; exit 1; fi
echo "binary of $(stat -c %y $NEW/n42 | cut -c1-16), tree $(git rev-parse --short HEAD)"
BUILD_FROM=0
