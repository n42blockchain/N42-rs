#!/usr/bin/env bash
# loop177: the streamed graft on the follower alone (N42_FOLLOWER_GRAFT_STREAM=1). loop176 split the change in two:
# on the leader it costs (the fold falls 73 -> 43-49 ms but the parallel step rises 78-84 -> 143-159, the fold being
# memory bandwidth rather than schedulable work), while on a follower it pays (import 344-363 -> 330-355 ms). The
# import gates the vote and the build after a tenure change, so this asks for that side on its own, three pairs.
# T0 = today, T1 = the follower's graft streamed. Tests and the build run BEFORE the claim; released on any exit.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop177.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
run() { local tag=$1; shift
  ...
  e=$(pgrep -f '/n4[2] node' | head -1); [ -n "$e" ] && echo "el binary: $(readlink /proc/$e/exe) env: $(tr '\0' '\n' < /proc/$e/environ | grep -E 'QMDB_READS|HASHED_TABLES|GRAFT_STREAM|FOLLOWER_SENDER_GROUPS|FOLLOWER_GRAFT_STREAM|DIRECT_RECEIPTS|MALLOC_CONF' | paste -sd' ')"
  wait
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop177$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop177$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
FS='N42_FOLLOWER_GRAFT_STREAM=1'
leg T0a F7_BIN=$NEW $C $R $D $A2
leg T1a F7_BIN=$NEW $C $R $D $A2 $FS
leg T0b F7_BIN=$NEW $C $R $D $A2
leg T1b F7_BIN=$NEW $C $R $D $A2 $FS
leg T0c F7_BIN=$NEW $C $R $D $A2
leg T1c F7_BIN=$NEW $C $R $D $A2 $FS
cleanup
