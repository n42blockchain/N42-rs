#!/usr/bin/env bash
# loop178: the thread budget. Each node is pinned to 32 logical CPUs and runs a 16-thread build pool, a 16-thread
# global rayon pool (the follower's import), 8 tokio workers and 20 ingest recovery threads -- 60 threads on 32
# CPUs. The leader's parallel step is 78-84 ms for 163,000 transfers and a follower's execution 137-151, both of
# them rayon work, so what they are actually waiting for may be each other. P0 = today, P1 = 24-thread build and
# global pools, P2 = today's pools with the ingest recovery cut to 12, two legs each. The follower's streamed graft
# is now the default and is in every leg. Tests and the build run BEFORE the claim; released on any exit.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop178.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop178$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop178$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
WIDE='RAYON_NUM_THREADS=24 N42_PARALLEL_BUILD_THREADS=24'
THIN='N42_TX_INGEST_RECOVER_PARALLEL=12'
leg P0a F7_BIN=$NEW $C $R $D $A2
leg P1a F7_BIN=$NEW $C $R $D $A2 $WIDE
leg P2a F7_BIN=$NEW $C $R $D $A2 $THIN
leg P0b F7_BIN=$NEW $C $R $D $A2
leg P1b F7_BIN=$NEW $C $R $D $A2 $WIDE
leg P2b F7_BIN=$NEW $C $R $D $A2 $THIN
cleanup
