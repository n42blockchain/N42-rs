#!/usr/bin/env bash
# loop179: confirming loop178's P2 -- the ingest's recovery threads cut from 20 to 12 gave the best round and window
# of that loop (358k / 25.02M against 349k / 24.58M) and made the ingest itself faster per transaction (16 -> 12 us),
# by leaving the build and the import the CPUs they were short of. One of its legs left the box 2.6 GB free, so the
# memory floor is a criterion here, not a note: a leg that dips under 10 GB is reported as such. R0 = today's 20,
# R1 = 12, R2 = 8, two legs each, on the adopted allocator, the direct receipts and the follower's streamed graft.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop179.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop179$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop179$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
R12='N42_TX_INGEST_RECOVER_PARALLEL=12'
R8='N42_TX_INGEST_RECOVER_PARALLEL=8'
leg R0a F7_BIN=$NEW $C $R $D $A2
leg R1a F7_BIN=$NEW $C $R $D $A2 $R12
leg R2a F7_BIN=$NEW $C $R $D $A2 $R8
leg R0b F7_BIN=$NEW $C $R $D $A2
leg R1b F7_BIN=$NEW $C $R $D $A2 $R12
leg R2b F7_BIN=$NEW $C $R $D $A2 $R8
cleanup
