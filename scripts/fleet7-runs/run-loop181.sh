#!/usr/bin/env bash
# loop181: fewer threads a node. Pinned to contiguous CPUs -- which is how the fleet runs -- both parallel phases
# saturate at about eight threads in the bench: the leader's execution reads 78 ms at 8 threads, 88 at 16, 94 at 32
# and 101 at 64, and a follower's import 94-96 at 8 against 93-98 at 16 and 32. The work is bounded by memory and
# the L3, not by cores, and this box is one socket of 128 cores at 52% of its clock. Each node currently runs a
# 16-thread build pool and a 16-thread global pool on 32 logical CPUs. T0 = today, T1 = both pools at 8, T2 = both
# at 8 with the ingest's recovery at 8 as well, two legs each; the freed CPUs are what the ingest and the imports
# were short of (loop178/179). Tests and the build run BEFORE the claim; released on any exit.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop181.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
run() { local tag=$1; shift
  ...
  e=$(pgrep -f '/n4[2] node' | head -1); [ -n "$e" ] && echo "el binary: $(readlink /proc/$e/exe) env: $(tr '\0' '\n' < /proc/$e/environ | grep -E 'QMDB_READS|HASHED_TABLES|GRAFT_STREAM|FOLLOWER_GRAFT_STREAM|RAYON_NUM_THREADS|PARALLEL_BUILD_THREADS|RAYON_NUM_THREADS|PARALLEL_BUILD_THREADS|INGEST_RECOVER_PARALLEL|DIRECT_RECEIPTS|MALLOC_CONF' | paste -sd' ')"
  wait
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop181$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop181$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=20 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
T8='RAYON_NUM_THREADS=8 N42_PARALLEL_BUILD_THREADS=8'
I8='N42_TX_INGEST_RECOVER_PARALLEL=8'
leg T0a F7_BIN=$NEW $C $R $D $A2
leg T1a F7_BIN=$NEW $C $R $D $A2 $T8
leg T2a F7_BIN=$NEW $C $R $D $A2 $T8 $I8
leg T0b F7_BIN=$NEW $C $R $D $A2
leg T1b F7_BIN=$NEW $C $R $D $A2 $T8
leg T2b F7_BIN=$NEW $C $R $D $A2 $T8 $I8
cleanup
