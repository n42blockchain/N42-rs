#!/usr/bin/env bash
# loop182: step 0 of docs/FLEET7_PLAN_V4.md -- the pacing. The leader's build waits ~190 ms finished before it is
# wanted, R1 vote collection is 287 ms at the median, and the natural cycle (~330-350 ms) sits under the bench's 350 ms
# pacing, which was set at loop141 when the chain was slower; the measured cycle is 393-399. Legs at 350 / 300 / 275 /
# 250 ms. Expect the cycle to follow the pacing down to the natural one and stop; 300 collapsed at loop141 (handover
# stalls), so TCs past the start-up one are the failure sign. New counters on every leg: R1_collect, proposal@ and the
# view total from the leaders' logs -- their absence is how the vote path went unseen. Waits for loop181.
...
trap cleanup EXIT
# Not even a build while loop181 is measuring.
until grep -q ALLDONE $S/run-loop181.out 2>/dev/null; do sleep 60; done
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop182.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop182$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop182$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
leg P350a F7_BIN=$NEW $C $R $D $A2 F7_BLOCK_INTERVAL_MS=350
leg P300a F7_BIN=$NEW $C $R $D $A2 F7_BLOCK_INTERVAL_MS=300
leg P275a F7_BIN=$NEW $C $R $D $A2 F7_BLOCK_INTERVAL_MS=275
leg P250a F7_BIN=$NEW $C $R $D $A2 F7_BLOCK_INTERVAL_MS=250
leg P300b F7_BIN=$NEW $C $R $D $A2 F7_BLOCK_INTERVAL_MS=300
leg P275b F7_BIN=$NEW $C $R $D $A2 F7_BLOCK_INTERVAL_MS=275
cleanup
