#!/usr/bin/env bash
# loop183: plan v4 steps 1 and 2 on the fleet, at the 275 ms pacing loop182 settled on (window 1 372k against 358k at
# 350; every pacing from 350 to 250 held). V0 = today. V1 = N42_CHECK_ON_PARENT_OUTPUT=1: the parent's output is
# published when its execution ends and the sender check runs before the parent's fields are waited for. V2 =
# N42_FOLLOWER_EXEC_ON_PARENT_OUTPUT=1: a follower executes N+1 on N's output, the vote road and the execution side by
# side. Judged by R1 and the view's total first, then the cycle; any invalid block, gas-used mismatch or root
# disagreement (scripts/fleet7-verify.py, run against the live fleet at the end of each leg) fails the step. The
# tmpfs still holds 22 GB of stale ETL buffers unless someone has removed them: the memory floor is printed per leg.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop183.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop183$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop183$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
leg V0a F7_BIN=$NEW $C $R $D $A2
leg V1a F7_BIN=$NEW $C $R $D $A2 N42_CHECK_ON_PARENT_OUTPUT=1
leg V2a F7_BIN=$NEW $C $R $D $A2 N42_FOLLOWER_EXEC_ON_PARENT_OUTPUT=1
leg V0b F7_BIN=$NEW $C $R $D $A2
leg V1b F7_BIN=$NEW $C $R $D $A2 N42_CHECK_ON_PARENT_OUTPUT=1
leg V2b F7_BIN=$NEW $C $R $D $A2 N42_FOLLOWER_EXEC_ON_PARENT_OUTPUT=1
cleanup
