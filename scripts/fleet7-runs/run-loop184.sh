#!/usr/bin/env bash
# loop184: who holds the memory. Three of loop183's six legs bottomed under 12 GB free with 22 GB more to start from
# than before (2.7 GB at the worst, 11 s before a node's persistence stalled and its read view died), and the floor
# does not follow the configuration. The sampler here records, every 5 s through the leg, resident memory by process
# class (the seven execution layers: sum, largest and smallest; validators; the flood) beside the kernel's own
# split (anonymous, huge pages, page cache, shmem, slab, page tables) and the tmpfs. Three legs of the adopted
# configuration at the 275 ms pacing, the check on the parent's output on. Read it with the leg's min_avail_g.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop184.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop184$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop184$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
D='N42_QMDB_READS=on N42_HASHED_TABLES=off F7_BLOCK_INTERVAL_MS=275 N42_CHECK_ON_PARENT_OUTPUT=1'
leg M0a F7_BIN=$NEW $C $R $D $A2
leg M0b F7_BIN=$NEW $C $R $D $A2
leg M0c F7_BIN=$NEW $C $R $D $A2
cleanup
