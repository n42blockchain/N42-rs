#!/usr/bin/env bash
# loop186: the first round that stays full. Every round since the chain passed ~2.5 blocks a second has ended in
# window 3 with the flood priced out: its fee cap is 1e14 wei, full blocks raise the base fee 12.5% each, and ~156
# full blocks from a round's starting fee reach it -- occupancy 53-55% from there, and a round total of 156 full
# blocks whatever was being measured. G0 = the old cap, for the record; G1 = --gasprice 1e24 (~412 full blocks).
# Two legs each at the adopted configuration. Judged by occupancy in every window first, then the round; window 2's
# cycle (0.57-0.60 s against 0.41 in window 1, at 100% occupancy) is the real decay and is read with the memory.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop186.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop186$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop186$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
leg G0a F7_BIN=$NEW $C $R $D $A2
leg G1a F7_BIN=$NEW $C $R $D $A2 F7_GASPRICE_ARG=1000000000000000000000000
leg G0b F7_BIN=$NEW $C $R $D $A2
leg G1b F7_BIN=$NEW $C $R $D $A2 F7_GASPRICE_ARG=1000000000000000000000000
cleanup
