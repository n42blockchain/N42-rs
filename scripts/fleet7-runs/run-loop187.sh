#!/usr/bin/env bash
# loop187: persistence is the sustained ceiling -- one saturated save thread at ~2.0 full blocks a second against a
# chain that wants 1.9-2.5, its backlog paid in resident memory until seven nodes are 109 GB of 136 (loop186). Every
# leg here stays full to the end (--gasprice 1e24) and scrapes each node's metrics before the fleet goes down, so the
# save's stages are read from this build's own histograms (the last leg with a scrape was three weeks old).
# K1 = the adopted configuration with F7_SENDER_CACHE_MULT=1 (1 GB a node back; the 4x was inside the noise by the
# code's own comment). K2 = K1 + --prune.transaction-lookup.full: 163,000 TransactionHashNumbers puts a block leave
# the RocksDB batch and the MDBX leg, the largest single write of a save and the memtables' growth; by-hash RPC
# lookups stop working, which no part of the harness uses. K3 = K2 + N42_ROCKSDB_NOSYNC=1 (bench only: isolates how
# much of the commit is the WAL sync on a disk seven nodes share). Judged by the reader lag at the end of the load,
# the execution layers' peak, the free floor, and windows 2-3.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop187.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
PYEOF
  ...
  e=$(pgrep -f '/n4[2] node' | head -1); [ -n "$e" ] && echo "el binary: $(readlink /proc/$e/exe) env: $(tr '\0' '\n' < /proc/$e/environ | grep -E 'QMDB_READS|HASHED_TABLES|SENDER_CACHE|ROCKSDB_NOSYNC|BLOCK_INTERVAL|RAYON_NUM_THREADS|PARALLEL_BUILD_THREADS|INGEST_RECOVER_PARALLEL|DIRECT_RECEIPTS|MALLOC_CONF' | paste -sd' ')"
  wait
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop187$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop187$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
leg K1a F7_BIN=$NEW $C $R $D $A2
leg K2a F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full"
leg K3a F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full" N42_ROCKSDB_NOSYNC=1
leg K1b F7_BIN=$NEW $C $R $D $A2
leg K2b F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full"
leg K3b F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full" N42_ROCKSDB_NOSYNC=1
cleanup
