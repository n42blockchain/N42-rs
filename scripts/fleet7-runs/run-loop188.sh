#!/usr/bin/env bash
# loop188: defect 10's fix on the fleet, and the configuration loop187 arrived at. The queue gave a mined block's
# transactions back at a tenure handover (169,293 of them, never removed), every later build of that leader refused
# them for a stale nonce, the early seal went and the serial tail waited 2.8 s in the puller: 2 legs of 6. The fix
# (aefd9384d) keeps a mined-nonce watermark per lane that every way into the lanes checks, and makes a stale-nonce
# refusal final. Six legs, because the defect fired in a third of them: Q2 = --prune.transaction-lookup.full,
# Q3 = Q2 + N42_ROCKSDB_NOSYNC=1, three each, all full to the end (fee cap 1e24), sender-cache multiplier 1. A leg
# passes if no window's cycle leaves the 0.4-0.6 s band; the give-backs that filtered anything and the stale-nonce
# refusals are counted per leg.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop188.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop188$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop188$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
for r in a b c; do
  leg Q2$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full"
  leg Q3$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full" N42_ROCKSDB_NOSYNC=1
done
cleanup
