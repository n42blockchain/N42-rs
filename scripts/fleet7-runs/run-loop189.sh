#!/usr/bin/env bash
# loop189: plan v4 step 2, judged again now that its premise holds. loop183 found the two roads overlapping by 0-1 ms:
# at that cycle the parent's fields were filed before the child's check passed. Since then the cycle went 0.43 ->
# 0.375-0.40 s in window 1 (defect 10 fixed, persistence keeping up), a follower's import is ~400 ms -- not shorter
# than the cycle -- and R1 reads 259-297 ms against a follower's state-dependent chain of execution 141-167 + root 33
# + engine insert 52. So the child now arrives while its parent is still importing, which is what the step is for.
# X0 = the confirmed configuration (loop188 Q2), X1 = X0 + N42_FOLLOWER_EXEC_ON_PARENT_OUTPUT=1, three pairs, full to
# the end. Judged by R1 and the view's total, then the windows; the two roads' overlap is printed; any invalid block,
# gas-used mismatch or disagreement in fleet7-verify fails the step.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop189.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop189$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop189$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
for r in a b c; do
  leg X0$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full"
  leg X1$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full" N42_FOLLOWER_EXEC_ON_PARENT_OUTPUT=1
done
cleanup
