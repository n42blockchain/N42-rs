#!/usr/bin/env bash
# loop190: the vote road, shortened (plan v4 section 2i). Two changes on the confirmed configuration (loop188 Q2):
#  - check_includable in one pass over the block (unconditional in this build; 8.8 -> 5.1 ms on an idle pinned box,
#    11.1 -> 4.9 ms of total CPU) -- read from the new `vote road` line's check_ms, and against loop190 X0 across runs;
#  - N42_BODY_ONCE=1: the validator hands the execution layer the gossip body as received and the execution layer
#    decodes it once (FOREIGN_BODY), instead of decode + re-encode into NEW_PAYLOAD + 26 MB over the socket + parse.
#    The dissection put hand-off at 51.5 ms and convert at 55 ms; the claim is that most of both goes.
# Y0 = confirmed configuration, Y1 = Y0 + N42_BODY_ONCE=1, three pairs. Judged by the vote road's total and R1, then
# the view's total and the windows. Falsified if R1 does not fall by at least 40 ms with no fallback in use; any
# invalid block, gas-used mismatch, body refused into the fallback, or disagreement in fleet7-verify fails the step.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop190.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop190$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop190$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
for r in a b c; do
  leg Y0$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full"
  leg Y1$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full" N42_BODY_ONCE=1
done
cleanup
