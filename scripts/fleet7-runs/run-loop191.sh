#!/usr/bin/env bash
# loop191: the commit forkchoice off the service loop (plan v4 section 2i, segment D: 37 of its 51 ms were the
# leader's own commit forkchoice, awaited in the loop). N42_COMMIT_FCU_ASYNC=1 sends it from a task, one in flight, in
# commit order, the answer applied as a report.
# Z0 = the confirmed configuration, Z1 = Z0 + N42_COMMIT_FCU_ASYNC=1, Z2 = Z1 + N42_BODY_ONCE=1 (loop191: the body road
# cut R1 by ~40 ms at the median but not the cycle; whether it counts once D is shorter is what Z2 asks). Two rounds.
# Judged by proposal=@ in the leader's consensus_timing line (spans D+E+A; 263 ms in every loop191 leg), then the
# view's total and window 1. Falsified if proposal=@ does not fall by at least 20 ms. Any invalid block, TC beyond the
# start-up one, refused commit forkchoice, or disagreement in fleet7-verify fails the step.
...
if ! nice -n 19 cargo build -j8 --release --target-dir target/deferred -p n42 --bin n42 -p n42-h2-node --example h2_validator --example tx_flood --example h2_keygen --example send_tx > $S/build-loop191.log 2>&1; then echo "BUILD FAILED"; echo ALLDONE; exit 1; fi
...
leg() {
  if [ $(( $(date -u +%s) - CLAIMED_AT )) -gt 4500 ]; then echo "leg loop191$1 skipped: 75 minutes of the claim used"; return; fi
  local tag=$1; shift
  run loop191$tag F7_EL_EXTRA="--builder.interval 60 --builder.deadline 3" N42_FAST_TRANSFER=1 N42_FOLLOWER_DIRECT_IMPORT=1 F7_SENDER_CACHE_MULT=4 N42_TX_QUEUE_BATCH=1024 N42_TX_QUEUE_DRAINER=1 F7_FLOOD_WINDOW=6 N42_TX_INGEST_RECOVER_PARALLEL=12 F7_BLOCK_INTERVAL_MS=350 "$@"
}
...
for r in a b; do
  leg Z0$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full"
  leg Z1$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full" N42_COMMIT_FCU_ASYNC=1
  leg Z2$r F7_BIN=$NEW $C $R $D $A2 "F7_EL_EXTRA=--builder.interval 60 --builder.deadline 3 --prune.transaction-lookup.full" N42_COMMIT_FCU_ASYNC=1 N42_BODY_ONCE=1
done
cleanup
